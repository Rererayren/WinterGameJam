...
func _on_body_entered(body: Node2D) -> void:
	...
	if masks_collected >= 1:
		...
		show_timed_message(SkillMessageType.DOUBLE_JUMP, "You've unlocked Double Jump (Jump twice)!", 3.0)
	if masks_collected >= 2:
		player.set_super_dash()
		show_timed_message(SkillMessageType.SUPER_DASH, "You've unlocked Super Dash (Keyboard S or Joypad B)!", 3.0)
	if masks_collected >= 3:
		player.set_float()
		show_timed_message(SkillMessageType.FLOAT, "You've unlocked Slow Falling (Double Jump then hold)!", 3.0)
